...
def main():
	# Run tests against XenonMKV to confirm that everything is operational after code changes
	passes = 0
	tests = [
		"file_not_exists",
		"destdir_not_exists",
...
def test_invalid_mkv_file():
	# Pass an invalid file that contains a string of text and is not a valid MKV container.
	# Expected result: output contains 'CRITICAL' and prompts for a valid MKV file
...
def test_mkv_file_no_usable_tracks():
	...
	# Expected result: output contains 'CRITICAL' and specifically mentions no usable tracks. 
...
def test_mkv_file_no_video_track():
	...
	# Expected result: output contains 'CRITICAL' and specifically mentions lack of a video track.
...
def in_output(strings, output):
	if "Exception" in output:
		print "An exception was thrown in the output, failing test automatically. Details: "
		print output
		return False
