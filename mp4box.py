...
class MP4Box():
	...
	def __init__(self, video_path, audio_path, video_fps, video_pixel_ar, args, log):
	...
	def package(self):
		...
		while run_attempts <= self.args.mp4box_retries:
			cmd = [self.args.tool_paths["mp4box"], "output.mp4", "-add", self.video_path, "-fps", self.video_fps, "-par", "1=" + self.video_pixel_ar, "-add", self.audio_path, "-tmp", self.args.scratch_dir, "-itags", "name=" + self.args.name]
			...
			if process.returncode != 0:
				...
				os.unlink(output_file)
				self.log.warning("An error occurred while creating an MP4 file with MP4Box; %i retries left" % (self.args.mp4box_retries - run_attempts))
				run_attempts += 1
