...
class MKVTrack():
    ...
    # mplayer should play EAC3 with the correct codecs
    codec_table["A_EAC3"] = ".eac3"
    codec_table["A_AAC"] = ".aac"
    codec_table["A_AAC/MPEG2/LC"] = ".aac"
    codec_table["A_DTS"] = ".dts"
    codec_table["A_MP3"] = ".mp3"
    codec_table["A_MPEG/L3"] = ".mp3"
    # MS/ACM: because people like to run anime through this thing
    codec_table["A_MS/ACM"] = ".mp3"
    ...
    def get_filename_extension(self):
        ...
        if self.codec_id in ('DIV3', 'WVC1', 'V_MS/VFW/FOURCC / WVC1'):
            ...
            raise UnsupportedCodecError("%s codec in selected video track "
                "is not supported in an MP4 container" % self.codec_id)

        # Otherwise, set defaults
        if self.codec_id.startswith("A_"):
            self.log.warning("Returning default .aac extension for audio "
                "codec %s" % self.codec_id)
            return ".aac"
        ...
        raise Exception("Could not detect appropriate extension for codec %s" %
            self.codec_id)
